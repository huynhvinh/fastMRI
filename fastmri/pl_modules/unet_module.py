...
class FixMatchUnetModule(MriModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        label_ce_loss, unlabelled_loss = torch.tensor(0.0, requires_grad=True, device=self.device), torch.tensor(0.0, requires_grad=True)
        # labelled images
        if(slice_index > 0):
        ...
        if slice_index < n:
            ...
            unlabel_ce_loss = unlabelled_loss * unlabel_mask
    ...
    def validation_step(self, batch, batch_idx):
        ...
        label_ce_loss, unlabelled_loss = torch.tensor(0.0, requires_grad=True, device=self.device), torch.tensor(0.0, requires_grad=True, device=self.device)
        # labelled images
        if(slice_index > 0):
            label_op, label_ft = self(strong_img[:slice_index])
            label_ce_loss = F.l1_loss(label_op, target[:slice_index])
...
class UnetBarlowModule(MriModule):
    """
    Unet training module.
...
class UnetModule(MriModule):
    ...
    def forward(self, image):
        return self.unet(image.unsqueeze(1))[0].squeeze(1)
